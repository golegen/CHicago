...
// Last edited on January 21 of 2019, at 12:09 BRT
...
static Boolean SubarchSetVideoMode(UInt16 width, UInt16 height, PUInt64 data) {
	EFI_HANDLE *hbuf;
	...
	if (data != Null) {																																			// Set out data?
		...
		data[1] = height;
		data[2] = 32;
		data[3] = gop->Mode->FrameBufferBase;
	}
